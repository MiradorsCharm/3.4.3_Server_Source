 ...
 * loop once started; the script adds Serpent Sting, Arcane/Steady/Multi
 * Shot, aspect upkeep, pet care (revive + mend) and keeps the bot out of
 * the (weapon-dependent) minimum range by retreating.
 */
...
namespace
{
    ...
    constexpr uint32 STEADY_SHOT[] = { 34120, 49051, 49052 };
...
public:
    ...
    {
        // a dead pet is useless - bring it back while we are safe
        if (Pet* pet = ai.GetBot()->GetPet())
            if (!pet->IsAlive())
                CastOnSelf(Rank(REVIVE_PET));
    }
    ...
    void HealTick(BotAI& ai) override
    {
        ...
        if (Pet* pet = ai.GetBot()->GetPet())
            if (pet->IsAlive() && pet->GetHealthPct() < 50.0f)
    ...
    void CombatTick(BotAI& ai) override
    {
        ...
        // a dead pet during the fight gets revived on the spot
        if (Pet* pet = ai.GetBot()->GetPet())
            if (!pet->IsAlive() && CastOnSelf(Rank(REVIVE_PET)))
                return;
