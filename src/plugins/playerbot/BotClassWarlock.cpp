 ...
 *
 * DoT-and-nuke rotation (Immolate/Corruption/Curse into Shadow Bolt), demon
 * upkeep, Life Tap mana management, Health Funnel pet care, Fear escapes
 * and Fel Armor/Healthstone self-care.
 */
...
namespace
{
    ...
    constexpr uint32 CURSE_OF_AGONY[] = { 980, 1014, 6217, 11711, 11712, 11713, 27218, 47863, 47864 };
    ...
    constexpr uint32 DRAIN_LIFE[] = { 689, 699, 709, 7651, 11693, 11694, 11695, 47841, 47842, 47843 };
    ...
    constexpr uint32 FEL_ARMOR[] = { 28189, 47893 };
    constexpr uint32 SUMMON_IMP[] = { 688 };
    constexpr uint32 SUMMON_VOIDWALKER[] = { 697 };
    constexpr uint32 SUMMON_FELHUNTER[] = { 691 };
    constexpr uint32 HEALTH_FUNNEL[] = { 755, 3698, 3699, 3700, 11692, 11693, 11694, 11695, 27259, 47856, 47857 };
    constexpr uint32 FEAR[] = { 5782, 6213, 6215, 6219, 710, 7645 };
    constexpr uint32 SOUL_FIRE[] = { 6353, 17951, 17952 };
}
...
public:
    ...
    {
        Player* bot = ai.GetBot();
        if (bot->GetPower(POWER_MANA) < bot->GetMaxPower(POWER_MANA) / 6)
            CastOnSelf(Rank(LIFE_TAP));
        if (bot->GetHealthPct() < 65.0f)
            CastOnVictim(Rank(DRAIN_LIFE));
        if (Pet* pet = bot->GetPet())
            if (pet->IsAlive() && pet->GetHealthPct() < 40.0f)
                CastOnSelf(Rank(HEALTH_FUNNEL));
    }
    ...
    void CombatTick(BotAI& ai) override
    {
        ...
        // flee button when the fight turns against us
        if (ai.GetBot()->GetHealthPct() < 25.0f && CastOnVictim(Rank(FEAR)))
            return;
