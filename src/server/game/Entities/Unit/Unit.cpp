...
DamageInfo::DamageInfo(DamageInfo const& dmg1, DamageInfo const& dmg2)
    : m_attacker(dmg1.m_attacker), 
    m_victim(dmg1.m_victim), 
    m_damage(dmg1.m_damage + dmg2.m_damage),
    m_originalDamage(dmg1.m_originalDamage + dmg2.m_originalDamage),
    m_spellInfo(dmg1.m_spellInfo), 
    m_schoolMask(SpellSchoolMask(dmg1.m_schoolMask | dmg2.m_schoolMask)),
    m_damageType(dmg1.m_damageType), 
    m_attackType(dmg1.m_attackType), 
    m_absorb(dmg1.m_absorb + dmg2.m_absorb), 
    m_resist(dmg1.m_resist + dmg2.m_resist), 
    m_block(dmg1.m_block), 
    m_hitMask(dmg1.m_hitMask | dmg2.m_hitMask)
{
}
...
DamageInfo::DamageInfo(CalcDamageInfo const& dmgInfo, uint8 damageIndex)
    ...
    m_damage(dmgInfo.Damages[damageIndex].Damage),
    m_originalDamage(dmgInfo.Damages[damageIndex].Damage),
    m_spellInfo(nullptr), 
    m_schoolMask(SpellSchoolMask(dmgInfo.Damages[damageIndex].DamageSchoolMask)),
...
DamageInfo::DamageInfo(SpellNonMeleeDamage const& spellNonMeleeDamage, DamageEffectType damageType, WeaponAttackType attackType, ProcFlagsHit hitMask)
    ...
    m_originalDamage(spellNonMeleeDamage.originalDamage),
...
SpellNonMeleeDamage::SpellNonMeleeDamage(Unit* _attacker, Unit* _target, SpellInfo const* _spellInfo, SpellCastVisual spellVisual, uint32 _schoolMask, ObjectGuid _castId)
    ...
    cleanDamage(0), 
    fullBlock(false), 
    preHitHealth(_target->GetHealth())
{
}
...
Unit::Unit(bool isWorldObject) 
    ...
    LastCharmerGUID(), 
    movespline(std::make_unique<Movement::MoveSpline>()),
    ...
    chaseRepositionTimer(5 * IN_MILLISECONDS)
{
    ...
    changedIntoShapeshift = false;

    m_IsFleeingForAssist = false;
    m_JustFledForAssist = false;
}
...
void Unit::Update(uint32 p_time)
{
    ...
    if (IsInCombat())
    {
        if (chaseRepositionTimer <= p_time)
        {
            chaseRepositionTimer = 2 * IN_MILLISECONDS;
            UpdateChaserCombatPosition();
        }
        else
            chaseRepositionTimer -= p_time;
    }
...
bool Unit::IsWithinMeleeRangeAt(Position const& pos, Unit const* obj, bool leeway) const
{
    if (!obj || !IsInMap(obj) || !InSamePhase(obj))
        return false;
...
/*static*/ uint32 Unit::DealDamage(Unit* attacker, Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss)
{
        ...
        if (!damageTaken && damagetype != DOT && cleanDamage && cleanDamage->absorbed_damage)
            if (victim != attacker && victim->GetTypeId() == TYPEID_PLAYER)
                if (Spell* spell = victim->m_currentSpells[CURRENT_GENERIC_SPELL])
                    if (spell->getState() == SPELL_STATE_PREPARING && spell->m_spellInfo->InterruptFlags.HasFlag(SpellInterruptFlags::DamageCancelsPlayerOnly))
    ...
    if (attacker && cleanDamage && damagetype == DIRECT_DAMAGE && attacker != victim && attacker->GetPowerType() == POWER_RAGE)
    {
        ...
        switch (cleanDamage->attackType)
        {
            ...
            case OFF_ATTACK:
            {
                weaponSpeedHitFactor = uint32(attacker->GetBaseAttackTime(cleanDamage->attackType) / 1000.0f * (cleanDamage->attackType == BASE_ATTACK ? 3.5f : 1.75f));
...
void Unit::CalculateMeleeDamage(Unit* victim, CalcDamageInfo* damageInfo, WeaponAttackType attackType /*= BASE_ATTACK*/, bool forceCrit /*= false*/)
{
    ...
    for (uint8 i = 0; i < MAX_ITEM_PROTO_DAMAGES; ++i)
    {
        ...
        damageInfo->Damages[i].OriginalDamage   = 0;
    ...
    switch (damageInfo->HitOutCome)
    {
        case MELEE_HIT_EVADE:
        {
            ...
            for (uint8 i = 0; i < MAX_ITEM_PROTO_DAMAGES; ++i)
            {
                damageInfo->Damages[i].OriginalDamage = damageInfo->Damages[i].Damage;
                damageInfo->Damages[i].Damage = 0;
            }
...
/*static*/ float Unit::CalculateAverageResistReduction(WorldObject const* caster, SpellSchoolMask schoolMask, Unit const* victim, SpellInfo const* spellInfo /*= nullptr*/)
{
    ...
    const uint32 level = std::max(caster ? victim->GetLevelForTarget(caster) : victim->GetLevel(), uint8(20));
...
/*static*/ void Unit::CalcAbsorbResist(DamageInfo& damageInfo, Spell* spell /*= nullptr*/)
{
    ...
    if (damageInfo.GetVictim() != damageInfo.GetAttacker())
    {
        // We're going to call functions which can modify content of the list during iteration over it's elements
        // Let's copy the list so we can prevent iterator invalidation
        AuraEffectVector vSplitDamageFlatCopy = CopyAuraEffectList(damageInfo.GetVictim()->GetAuraEffectsByType(SPELL_AURA_SPLIT_DAMAGE_FLAT));
        for (auto itr = vSplitDamageFlatCopy.begin(); (itr != vSplitDamageFlatCopy.end()) && (damageInfo.GetDamage() > 0); ++itr)
        {
            // Check if aura was removed during iteration - we don't need to work on such auras
...
void Unit::DoMeleeAttackIfReady()
{
    ...
    {
        if (!IsWithinMeleeRange(victim, IsPlayer()))
            return AttackSwingErr::NotInRange;

        //120 degrees of radiant range, if player is not in boundary radius
        // alistar: don't check for the IsWithinBoundaryRadius makes it so we can do melee attacks while 
        // facing a creature with our back
...
void Unit::AttackerStateUpdate(Unit* victim, WeaponAttackType attType, bool extra)
{
    ...
    bool forceCrit = !victim->IsStandState();
    ...
    CalculateMeleeDamage(victim, &damageInfo, attType, forceCrit);
    // Send log damage message to client

    for (uint8 i = 0; i < MAX_ITEM_PROTO_DAMAGES; ++i)
        Unit::DealDamageMods(damageInfo.Attacker, victim, damageInfo.Damages[i].Damage, &damageInfo.Damages[i].Absorb);
...
MeleeHitOutcome Unit::RollMeleeOutcomeAgainst(Unit const* victim, WeaponAttackType attType) const
{
    ...
    if (crit_chance > 0 && !victim->IsStandState())
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit* victim, SpellInfo const* spellInfo) const
{
    ...
    uint32 missChance = uint32(MeleeSpellMissChance(victim, attType, skillDiff, spellInfo) * 100.0f);
    // Roll miss
...
uint32 Unit::GetWeaponSkillValue(WeaponAttackType attType, Unit const* target /*= nullptr*/) const
{
    uint32 value = 0;
    if (Player const* player = ToPlayer())
    {
...
AuraEffect* Unit::GetAuraEffect(AuraType type, SpellFamilyNames family, uint32 familyFlag1, uint32 familyFlag2, uint32 familyFlag3, ObjectGuid casterGUID /*= ObjectGuid::Empty*/) const
{
    AuraEffectList const& auras = GetAuraEffectsByType(type);
...
void Unit::UpdateResistanceBuffModsMod(SpellSchools school)
{
    ...
    SetUpdateFieldValue(m_values.ModifyValue(&Unit::m_unitData).ModifyValue(&UF::UnitData::ResistanceBuffModsPositive, school), int32(modPos));
    SetUpdateFieldValue(m_values.ModifyValue(&Unit::m_unitData).ModifyValue(&UF::UnitData::ResistanceBuffModsNegative, school), int32(modNeg));
}
...
void Unit::SendAttackStateUpdate(CalcDamageInfo* damageInfo)
{
    ...
    packet.OriginalDamage = damageInfo->Damages[0].OriginalDamage + damageInfo->Damages[1].OriginalDamage;
    ...
    packet.SubDmg->SchoolMask = damageInfo->Damages[0].DamageSchoolMask;   // School of sub damage
    packet.SubDmg->FDamage    = damageInfo->Damages[0].Damage;             // sub damage
    packet.SubDmg->Damage     = damageInfo->Damages[0].Damage;             // Sub Damage
    packet.SubDmg->Absorbed   = damageInfo->Damages[0].Absorb;
    packet.SubDmg->Resisted   = damageInfo->Damages[0].Resist;
    ...
    else if (Pet* pet = ToCreature()->ToPet())
    {
        if (pet->isControlled())
        {
            ...
            if (owner && (owner->GetTypeId() == TYPEID_PLAYER) && owner->ToPlayer()->GetGroup())
                owner->ToPlayer()->SetGroupUpdateFlag(GROUP_UPDATE_FLAG_CUR_POWER); // alistar: @TODO not sure about this look below it's probably gonna be GROUP_UPDATE_FLAG_PET_ something
...
int32 Unit::SpellDamageBonusDone(Unit* victim, SpellInfo const* spellProto, int32 pdamage, DamageEffectType damagetype, SpellEffectInfo const& spellEffectInfo, Optional<float> const& donePctTotal, uint32 stack /*= 1*/) const
{
            ...
            if (GetTypeId() == TYPEID_PLAYER)
            {
                ...
                for (auto itr = playerSpells.begin(); itr != playerSpells.end(); ++itr)
                {
                    ...
                    switch (itr->first)
                    {
                        ...
                        case 49636:
                        case 49638:
                            if (SpellInfo const* proto = sSpellMgr->GetSpellInfo(itr->first, DIFFICULTY_NONE))
...
float Unit::SpellDamagePctDone(Unit* victim, SpellInfo const* spellProto, DamageEffectType damagetype) const
{
    ...
    for (AuraEffectList::const_iterator i = mOverrideClassScript.begin(); i != mOverrideClassScript.end(); ++i)
    {
        ...
        switch ((*i)->GetMiscValue())
        {
            ...
            case 6928:
            {
                if (victim->HasAuraState(AURA_STATE_WOUNDED_35_PERCENT, spellProto, this))
            ...
            case 6925:
            case 6927:
                if (HasAuraState(AURA_STATE_WOUNDED_20_PERCENT, spellProto, this))
                    AddPct(DoneTotalMod, (*i)->GetAmount());
                break;
            ...
            {
                // Merciless Combat
                if ((*i)->GetSpellInfo()->IconFileDataId == 135294)
                {
    ...
    switch (spellProto->SpellFamilyName)
    {
        case SPELLFAMILY_MAGE:
            // Ice Lance
            if (spellProto->IconFileDataId == 135844)
            {
            ...
            if (spellProto->SpellFamilyFlags[0] & 0x20600021 || spellProto->SpellFamilyFlags[1] & 0x9000)
            {
                if (victim->HasAuraWithMechanic((1 << MECHANIC_SNARE) | (1 << MECHANIC_SLOW_ATTACK)))
                {
                    ...
                    {
                        if ((*i)->GetSpellInfo()->IconFileDataId == 236226)
                        {
                            AddPct(DoneTotalMod, (*i)->GetAmount());
                            break;
                        }
        ...
        case SPELLFAMILY_PRIEST:
            ...
            if (spellProto->SpellFamilyFlags[0] & 0x800000)
            {
                ...
                if (AuraEffect* aurEff = GetAuraEffect(SPELL_AURA_OVERRIDE_CLASS_SCRIPTS, SPELLFAMILY_PRIEST, 237566, 1))
                    // Increase Mind Flay damage if Shadow Word: Pain present on target
                    if (victim->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_PRIEST, 0x8000, 0, 0, GetGUID()))
                        AddPct(DoneTotalMod, aurEff->GetAmount());
            }
        ...
        case SPELLFAMILY_PALADIN:
            ...
            if ((spellProto->SpellFamilyFlags[1] & 0x400000) && spellProto->IconFileDataId == 135969)
            {
        ...
        case SPELLFAMILY_WARLOCK:
            ...
            if (spellProto->SpellFamilyFlags[1] & 0x00020040)
            {
                ...
                {
                    if (AuraEffect const* aurEFf = GetDummyAuraEffect(SPELLFAMILY_WARLOCK, 236297, EFFECT_0))
                        AddPct(DoneTotalMod, aurEFf->GetAmount());
                }
            }
        ...
        case SPELLFAMILY_DEATHKNIGHT:
            ...
            if (spellProto->SpellFamilyFlags[0] & 0x2)
                if (AuraEffect* aurEff = GetDummyAuraEffect(SPELLFAMILY_DEATHKNIGHT, 237526, 0))
                    AddPct(DoneTotalMod, aurEff->GetAmount());

            // Glacier Rot
            if (spellProto->SpellFamilyFlags[0] & 0x2 || spellProto->SpellFamilyFlags[1] & 0x6)
                if (AuraEffect* aurEff = GetDummyAuraEffect(SPELLFAMILY_DEATHKNIGHT, 136083, 0))
...
int32 Unit::SpellDamageBonusTaken(Unit* caster, SpellInfo const* spellProto, int32 pdamage, DamageEffectType damagetype) const
{
    ...
    {
        switch ((*i)->GetSpellInfo()->IconFileDataId)
        {
            // Cheat Death
            case 132285:
...
float Unit::SpellCritChanceDone(const Unit* victim, SpellInfo const* spellInfo, SpellSchoolMask schoolMask, WeaponAttackType attackType /*= BASE_ATTACK*/, bool isPeriodic /*= false*/) const
{
    ...
    if (victim && IsPlayer() && !victim->IsStandState() && !spellInfo->IsHealingSpell())
        return 100.0f;
    ...
    switch (spellInfo->DmgClass)
    {
        ...
        case SPELL_DAMAGE_CLASS_MAGIC:
        {
            const auto getPhysicalCritChance = [&]() -> float
            {
                if (Player const* thisPlayer = ToPlayer())
                {
                    // alistar: healthstone should be able to crit.
                    if (spellInfo->SpellFamilyName == SPELLFAMILY_WARLOCK &&
                        spellInfo->GetEffect(EFFECT_0).Effect == SPELL_EFFECT_HEAL)
                    {
                        return thisPlayer->m_activePlayerData->SpellCritPercentage[SPELL_SCHOOL_SHADOW];
                    }
                }
            ...
            const auto getMagicCritChance = [&]
            {
                if (Player const* thisPlayer = ToPlayer())
                    return thisPlayer->m_activePlayerData->SpellCritPercentage[AsUnderlyingType(GetFirstSchoolInMask(schoolMask))];
        ...
        case SPELL_DAMAGE_CLASS_MAGIC:
        {
            ...
            if (caster)
            {
                ...
                for (AuraEffect const* aurEff : mOverrideClassScript)
                {
                    ...
                    switch (aurEff->GetMiscValue())
                    {
                        ...
                        case 7917: // Glyph of Shadowburn
                            if (HasAuraState(AURA_STATE_WOUNDED_35_PERCENT, spellInfo, caster))
                ...
                switch (spellInfo->SpellFamilyName)
                {
                    case SPELLFAMILY_MAGE:
                        // Glyph of Fire Blast
                        if (spellInfo->SpellFamilyFlags[0] == 0x2 && spellInfo->IconFileDataId == 135807)
                    ...
                    case SPELLFAMILY_DRUID:
                        ...
                        if (HasAuraState(AURA_STATE_FAERIE_FIRE))
                            if (AuraEffect const* aurEff = caster->GetDummyAuraEffect(SPELLFAMILY_DRUID, 136033, 0))
                                crit_chance += aurEff->GetAmount();

                        // cumulative effect - don't break

                        // Starfire
                        if (spellInfo->SpellFamilyFlags[0] & 0x4 && spellInfo->IconFileDataId == 135753)
                        {
                            // Improved Insect Swarm
                            if (AuraEffect const* aurEff = caster->GetDummyAuraEffect(SPELLFAMILY_DRUID, 136045, 0))
                ...
                switch (spellInfo->SpellFamilyName)
                {
                    case SPELLFAMILY_DRUID:
                        ...
                        if (spellInfo->SpellFamilyFlags[0] & 0x00800000
                            && spellInfo->IconFileDataId == 132127
                            && HasAuraState(AURA_STATE_BLEED))
                        {
                            if (AuraEffect const* rendAndTear = caster->GetDummyAuraEffect(SPELLFAMILY_DRUID, 236164, 1))
...
int32 Unit::SpellHealingBonusDone(Unit* victim, SpellInfo const* spellProto, int32 healamount, DamageEffectType damagetype, SpellEffectInfo const& spellEffectInfo, Optional<float> const& donePctTotal, uint32 stack /*= 1*/) const
{
...
MountCapabilityEntry const* Unit::GetMountCapability(uint32 mountType) const
{
    ...
    // alistar: in the db2 90% of the areas have mountFlag 0. So, I guess we need add these flags in order to mount up.
    if (mountFlags == AreaMountFlags::None)
    {
        mountFlags = AreaMountFlags::AllowGroundMounts | AreaMountFlags::AllowSurfaceSwimmingMounts | AreaMountFlags::AllowUnderwaterSwimmingMounts;
    }
    // and for some places like Dalaran
    else if (mountFlags.HasFlag(AreaMountFlags::AllowFlyingMounts))
    {
        mountFlags |= AreaMountFlags::AllowGroundMounts | AreaMountFlags::AllowSurfaceSwimmingMounts | AreaMountFlags::AllowUnderwaterSwimmingMounts;
    }
...
void Unit::UpdateMountCapability()
{
    ...
    for (AuraEffect* aurEff : mounts)
    {
        ...
        if (!aurEff->GetAmount())
        {
            // alistar: don't remove the aura for now.. it causes some issues like Kessel Run quest
...
bool Unit::ValidatePower(Powers power, uint32& index) const
{
    if (power == POWER_HAPPINESS && IsPet())
        index = 3;

    if (index == MAX_POWERS || index >= MAX_POWERS_PER_CLASS)
        return false;
...
void Unit::UpdateSpeed(UnitMoveType mtype)
{
    ...
    switch (mtype)
    {
        ...
        case MOVE_FLIGHT:
        {
            ...
            if (GetTypeId() == TYPEID_UNIT)
                speed *= IsPet() ? 1.14286f : ToCreature()->GetCreatureTemplate()->speed_run; // at this point, MOVE_WALK is never reached
...
void Unit::FollowTarget(Unit* target)
{
    if (!target)
    {
        TC_LOG_ERROR("entities.unit", "Unit::FollowTarget: Unit ({}) tried to follow a non-existant target.", GetGUID().ToString().c_str());
        return;
    }
...
void Unit::setDeathState(DeathState s)
{
    ...
    if (s == JUST_DIED)
    {
        // alistar: It's only refreshed in Unit::Update for ALIVE states.
        // when we've just died we need to update the aura state
        ...
        ModifyAuraState(AURA_STATE_WOUND_HEALTH_20_80, false);
        ModifyAuraState(AURA_STATE_WOUND_HEALTH_35_80, false);

        ModifyAuraState(AURA_STATE_HEALTHY_75_PERCENT, false);
...
DiminishingLevels Unit::GetDiminishing(DiminishingGroup group) const
{
    ...
    if (!diminish.stack && GetMSTimeDiffToNow(diminish.hitTime) > 15 * IN_MILLISECONDS)
        return DIMINISHING_LEVEL_1;
...
{
    ShapeshiftForm form = GetShapeshiftForm();
    return form == FORM_CAT_FORM || form == FORM_BEAR_FORM || form == FORM_DIRE_BEAR_FORM;
}
...
{
    // alistar: POWER_HEALTH is -2 it doesn't go well when we try to access index -2 ...
    if (power == POWER_HEALTH)
        return GetHealth();

    uint32 powerIndex = GetPowerIndex(power);
    if (!ValidatePower(power, powerIndex))
        return 0;
...
void Unit::SetPower(Powers power, int32 val, bool withPowerUpdate /*= true*/)
{
    ...
    {
        // alistar: @TODO there's some information missing about the pet flag
        /*if (pet->isControlled())
        {
            Unit* owner = GetOwner();
            if (owner && (owner->GetTypeId() == TYPEID_PLAYER) && owner->ToPlayer()->GetGroup())
        ...
        if (hitMask & (PROC_HIT_CRITICAL | PROC_HIT_PARRY | PROC_HIT_DODGE | PROC_HIT_BLOCK))
        {
            ...
            if (isVictim)
            {
                ...
                if (hitMask & PROC_HIT_PARRY)
                {
                    ...
                    {
                        ModifyAuraState(AURA_STATE_DEFENSIVE_2, true);
                        StartReactiveTimer(REACTIVE_DEFENSE_2);
                    }
                    else
                    {
                        ModifyAuraState(AURA_STATE_DEFENSIVE, true);
...
void Unit::SendPetDismissTalk()
{
    ...
    petDismissSound.ModelID = GetNativeDisplayId();
    petDismissSound.ModelPosition = Position(GetPositionX(), GetPositionY(), GetPositionZ());
...
void Unit::AddComboPoints(Unit* target, int8 count)
{
    ...
    if (target && target != m_comboTarget)
    {
        ...
        SetPower(POWER_COMBO_POINTS, count);
        target->AddComboPointHolder(this);
    }
    else
        SetPower(POWER_COMBO_POINTS, std::max<int8>(std::min<int8>(GetPower(POWER_COMBO_POINTS) + count, 5), 0));
...
void Unit::SendComboPoints()
{
    ...
    if (Player* player = ToPlayer())
        player->SetUpdateFieldValue(player->m_values.ModifyValue(&Unit::m_unitData).ModifyValue(&UF::UnitData::ComboTarget), m_comboTarget ? m_comboTarget->GetGUID() : ObjectGuid::Empty);
}
...
void Unit::ClearAllReactives()
{
    ...
    if (GetClass() == CLASS_HUNTER && HasAuraState(AURA_STATE_DEFENSIVE_2))
...
/*static*/ void Unit::Kill(Unit* attacker, Unit* victim, bool durabilityLoss /*= true*/, bool /*skipSettingDeathState*/ /*= false*/)
{
    ...
    if (isRewardAllowed && player && player != victim)
    {
        ...
        if (creature)
        {
            ...
            if (uint32 lootid = creature->GetCreatureDifficulty()->LootID)
    ...
    {
        if (Player* victimPlr = victim->ToPlayer())
        {
    ...
    if (player)
    {
        ...
        if (pet && pet->IsAlive() && pet->isControlled())
        {
            if (pet->IsAIEnabled())
                ...
                TC_LOG_ERROR("entities.unit", "Pet doesn't have any AI in Unit::Kill(). {}", pet->GetDebugInfo().c_str());
        }
    }
    ...
    if (Player* plrVictim = victim->ToPlayer())
    {
        ...
        if ((durabilityLoss && !player && !victim->ToPlayer()->InBattleground()) || (player && sWorld->getBoolConfig(CONFIG_DURABILITY_LOSS_IN_PVP)))
        {
            ...
            plrVictim->SendDurabilityLoss(plrVictim, baseLoss);
        }
    ...
    else
    {
        switch (state)
        {
            case UNIT_STATE_STUNNED:
                ...
                break;
            case UNIT_STATE_ROOT:
                if (HasAuraType(SPELL_AURA_MOD_ROOT) || HasAuraType(SPELL_AURA_MOD_ROOT_2) || GetVehicle() || (ToCreature() && ToCreature()->IsTemplateRooted()))
                    return;
    ...
    if (playerCharmer)
    {
        switch (type)
        {
            case CHARM_TYPE_VEHICLE:
                ...
                playerCharmer->SetClientControl(this, true, true);
    ...
    if (playerCharmer)
    {
        switch (type)
        {
            case CHARM_TYPE_VEHICLE:
                ...
                playerCharmer->SetClientControl(charmer, true, true);
...
void Unit::PetSpellFail(const SpellInfo* spellInfo, Unit* target, uint32 result)
{
    ...
    // alistar: TODO IsPathfindingEnabled
        ...
        if ((result == SPELL_FAILED_LINE_OF_SIGHT || result == SPELL_FAILED_OUT_OF_RANGE) || !ToCreature()->HasReactState(REACT_PASSIVE))
        {
            if (Unit* owner = GetOwner())
            {
                ...
                else if (owner->IsValidAttackTarget(target))
                {
                    ...
                    {
                        // alistar: TODO this works well for instant casts but for spells with 
                        // cast time the pet just goes to target and returns without casting
                        m_attacking = target;
...
float Unit::MeleeSpellMissChance(Unit const* victim, WeaponAttackType attType, int32 skillDiff, SpellInfo const* spellInfo) const
{
...
uint32 Unit::GetModelForForm(ShapeshiftForm form, uint32 spellId) const
{
    ...
    // alistar: for shapeshift forms. Different hair color = different form model
    if (Player const* player = ToPlayer())
    {
        const uint32 skinColor = player->m_playerData->Customizations[0].ChrCustomizationChoiceID;
        const uint32 hairColor = player->m_playerData->Customizations[3].ChrCustomizationChoiceID;
...
void Unit::MoveAdvanceTo(Unit* target)
{
    ...
    if (GetCombatReach() * 0.25f > target->GetCombatReach())
        return;

    //! Too large of a Z difference
    if (ToCreature() && !CanFly() && (GetDistanceZ(target) > CREATURE_Z_ATTACK_RANGE + ToCreature()->m_CombatDistance))
        return;
...
void Unit::UpdateMovementForcesModMagnitude()
{
    if (Player* movingPlayer = GetPlayerMovingMe())
    {
        ...
        setModMovementForceMagnitude.Speed = 1.0f;
    ...
    else
    {
        ...
        updateModMovementForceMagnitude.Speed = 1.0f;
...
struct AttackerData
{
    ...
    bool operator<(const AttackerData& rhs) const
    {
        return angle < rhs.angle;
    }
};

bool IsAngleFree(std::vector<AttackerData> const& attackers, float angle, const float& max_angle)
{
    AttackerData data = { angle, 0.f, nullptr, 0 };
    auto low = std::lower_bound(attackers.begin(), attackers.end(), data);
    for (uint32 i = 0; i < 2; i++)
        if (low != attackers.end())
        {
            if (std::abs(angle - low->angle) < max_angle)
                return false;
            low++;
        }
    return true;
...
void RelocateAttackerToAngle(Unit* origin, const AttackerData& attacker, float angle)
{
    float x, y, z;
    ...
    init.MoveTo(x, y, z, true, false);
    init.SetWalk(true);
    init.SetFacing(origin);
    init.Launch();
}

void ProcessAttackerGroup(Unit* origin, std::vector<AttackerData> const& attackers, std::vector<AttackerData>::const_iterator start, std::vector<AttackerData>::const_iterator end, uint32 size, const float& max_angle)
{
    if (size < 2)
        return;

    auto itr = start;
    uint32 center = size / 2 - (size % 2 == 0 ? urand(0, 1) : 0);
    bool inInstance = origin->GetMap()->Instanceable();
    while (true)
    {
        uint32 index = itr - start;
        if (index != center && itr->unit->chaseRepositionCounter < 6)
        {
            std::vector<float> modifiers;
            if (index > center)
                modifiers = { 1.f, -1.f, 0.5f, -0.5f };
            else
                modifiers = { -1.f, 1.f, -0.5f, 0.5f };
            ...
            for (auto mod : modifiers)
            {
                ...
                angle += float(M_PI_4) * mod + frand(-0.1f, 0.1f);
                if (!IsAngleFree(attackers, angle, max_angle))
                    continue;

                if (!inInstance || std::abs(angle - itr->unit->chaseOriginalAngle) < float(M_PI_2))
                {
                    RelocateAttackerToAngle(origin, *itr, angle);
                    positionFound = true;
                    break;
                }
            }
            ...
            {
                float angle = itr->angle;
                angle += float(M_PI_4) * modifiers[0] + frand(-0.1f, 0.1f);
...
void Unit::UpdateChaserCombatPosition()
{
    const uint32 MAX_LAYERS = 4;
    std::vector<AttackerData> attacker_layer[MAX_LAYERS];
    uint32 nextIdx = 0;

    for (auto attacker : m_attackers)
    {
        if (attacker->GetMotionMaster()->GetCurrentMovementGeneratorType() == CHASE_MOTION_TYPE && attacker->movespline->Finalized()
            && !attacker->HasUnitState(UNIT_STATE_NOT_MOVE) && (attacker->GetTypeId() != TYPEID_UNIT
                                                                || (!attacker->ToCreature()->isWorldBoss() && !attacker->ToCreature()->IsDungeonBoss())))
        {
            AttackerData data = { GetAbsoluteAngle(attacker), GetExactDist(attacker), attacker, nextIdx++ };
            float dist = data.dist / DEFAULT_PLAYER_COMBAT_REACH;
            if (dist < 1.0)
                continue;
            uint32 layer = uint32(dist) - 1;
            if (layer >= MAX_LAYERS)
                continue;
            attacker_layer[layer].push_back(data);
        }
    }

    for (uint32 layer = 0; layer < MAX_LAYERS; layer++)
    {
        auto& attackers = attacker_layer[layer];

        if (attackers.size() < 2)
            continue;

        float max_angle = float(M_PI / 8.0) / float(layer + 1);

        std::sort(attackers.begin(), attackers.end());
        std::vector<AttackerData>::const_iterator nextItr, firstGroupLastItr, groupItr;
        std::vector<AttackerData>::const_iterator lastItr = attackers.begin();
        while (true)
        {
            nextItr = lastItr + 1;

            if (nextItr == attackers.end() || std::abs(nextItr->angle - lastItr->angle) >= max_angle)
            {
                groupItr = nextItr;
                break;
            }
        ...
        if (groupItr == attackers.end())
        {
            ProcessAttackerGroup(this, attackers, attackers.begin(), attackers.end() - 1, attackers.size(), max_angle);
            return;
        }

        firstGroupLastItr = lastItr;
        lastItr = groupItr;
        nextItr = groupItr + 1;

        while (nextItr != attackers.end())
        {
            if (std::abs(nextItr->angle - lastItr->angle) >= max_angle)
            {
                ProcessAttackerGroup(this, attackers, groupItr, lastItr, lastItr - groupItr + 1, max_angle);
                groupItr = nextItr;
            }
            lastItr = nextItr;
            nextItr++;
        ...
        if (std::abs(lastItr->angle - 2 * float(M_PI) - attackers.begin()->angle) >= max_angle)
        {
            ProcessAttackerGroup(this, attackers, groupItr, lastItr, lastItr - groupItr + 1, max_angle);
            ProcessAttackerGroup(this, attackers, attackers.begin(), firstGroupLastItr, firstGroupLastItr - attackers.begin() + 1, max_angle);
        }
        else
            ProcessAttackerGroup(this, attackers, groupItr, firstGroupLastItr, attackers.end() - groupItr + firstGroupLastItr - attackers.begin() + 1, max_angle);
    }
}
