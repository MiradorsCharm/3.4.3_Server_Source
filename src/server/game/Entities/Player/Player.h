...
constexpr float RUNE_MISS_COOLDOWN = 0.15f;
...
constexpr uint32 SPELL_DK_RAISE_ALLY = 46619;
...
enum PlayerFlagsEx
{
    ...
    PLAYER_FLAGS_EX_HIDE_HELM               = 0x0080,
    PLAYER_FLAGS_EX_HIDE_CLOAK              = 0x0100,
...
struct TC_GAME_API SpecializationInfo
{
    ...
    std::array<std::array<uint16, MAX_GLYPH_SLOT_INDEX>, MAX_SPECIALIZATIONS > Glyphs;
...
class TC_GAME_API Player final : public Unit, public GridObject<Player>
{
    ...
    public:
        ...
        void SendStable(ObjectGuid stableMaster);

        Pet* GetPet() const;
        Pet* SummonPet(uint32 entry, float x, float y, float z, float ang, PetType petType, uint32 despwtime);
        void RemovePet(Pet* pet, PetSaveMode mode, bool returnreagent = false, bool logout = false);
    ...
    public:
        ...
        uint8 FindEquipSlot(Item const* item, uint32 slot, bool swap, bool is_equipping = true) const;
        ...
        InventoryResult CanEquipItem(uint8 slot, uint16& dest, Item* pItem, bool swap, bool not_loading = true, bool is_equipping = true) const;
        ...
        void StoreLootItem(ObjectGuid lootWorldObjectGuid, uint8 lootSlot, Loot* loot);
        ...
        void ModifyCurrencyFromItemId(uint32 itemId, uint32 amount);
        ...
        bool AddItemWithToast(uint32 itemId, uint32 count);
        ...
        const Quest* GetNextAvailableQuest(const QuestMenu& questMenu);
        ...
        std::array<uint16, MAX_GLYPH_SLOT_INDEX>& GetGlyphs(uint8 talentGroupId) { return _specializationInfo.Glyphs[talentGroupId]; }
        std::array<uint16, MAX_GLYPH_SLOT_INDEX> const& GetGlyphs(uint8 talentGroupId) const { return _specializationInfo.Glyphs[talentGroupId]; }
        ...
        void BuildPlayerTalentsInfoData(WorldPackets::Talent::UpdateTalentData & packet);
        void BuildPetTalentsInfoData(WorldPackets::Talent::UpdateTalentData & packet);
    ...
    public:
        ...
        // Honor / Arena
        uint32 GetHonorPoints() const { return GetCurrencyQuantity(CURRENCY_TYPE_HONOR_POINTS); }
        uint32 GetArenaPoints() const { return GetCurrencyQuantity(CURRENCY_TYPE_ARENA_POINTS); }
        ...
        void SendCorpseReclaimDelayPacket(uint32 delay) const;
        ...
        void SetClientControl(Unit* target, bool allowMove, bool setMover = false);
        ...
        void SetLastTemporarySummonedPetNumber(uint32 petnumber) { m_LastTemporarySummonedPetNumber = petnumber; }
        ...
        float GetRuneBaseCooldown(uint8 index);
        ...
        void SetRuneCooldown(uint8 index, float cooldown, bool casted = false);
        ...
        void SetRestState(PlayerRestState state)
        {
            SetUpdateFieldValue(m_values
                .ModifyValue(&Player::m_activePlayerData)
                .ModifyValue(&UF::ActivePlayerData::RestInfo, 0)
                .ModifyValue(&UF::RestInfo::StateID), state);
        }
        void SetRestThreshold(uint32 threshold)
        {
            SetUpdateFieldValue(m_values
                .ModifyValue(&Player::m_activePlayerData)
                .ModifyValue(&UF::ActivePlayerData::RestInfo, 0)
        ...
        void SetTrackResourceFlag(uint8 type, uint32 flags) { SetUpdateFieldFlagValue(m_values.ModifyValue(&Player::m_activePlayerData).ModifyValue(&UF::ActivePlayerData::TrackResourceMask, type), flags); }
        void RemoveTrackResourceFlag(uint8 type, uint32 flags) { RemoveUpdateFieldFlagValue(m_values.ModifyValue(&Player::m_activePlayerData).ModifyValue(&UF::ActivePlayerData::TrackResourceMask, type), flags); }
    ...
    private:
        ...
        uint32 m_LastTemporarySummonedPetNumber;
